...
class Encoder(tf.keras.Model):
    ...
    def call(self, x, h1, h2):
        ...
        if self.rnn_type == 'gru' and self.bidirectional == False:
            output1, state1 = self.rnn1(x, initial_state=h1)
            output2, state2 = self.rnn2(self.W(output1), initial_state=h2)
            return output2, state1, state2

        output1 = self.rnn1(x, initial_state=h1)
        output2 = self.rnn2(self.W(output1[0]), initial_state=h2)
        # result, h1, h2
...
class Decoder(tf.keras.Model):
    ...
    def call(self, x, enc_output, h1, h2):
        ...
        output1 = self.rnn1(x)
        output2 = self.rnn2(self.W(output1[0]))
        state2 = output2[1]
        output = output2[0]
